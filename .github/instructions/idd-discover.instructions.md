...
2. If the target issue carries the configured roadmap label or an
   `lints-config-roadmap-id` marker — the same test
   **A2**'s roadmap-node/execution-leaf classification rule uses (an
   ...
   - Apply **A3**'s dependency bullet to the target itself (both
     ...
     `lints-config-blocked-by` markers) and its
...
`lints-config-roadmap-id` marker.

**Legacy roots**: `--all-roadmaps` finds roots only by label or
`lints-config-roadmap-id` marker. Retro-label a legacy
...
`<!-- lints-config-roadmap-id: ... -->` marker are
...
- **A0-T only**: the scoped body-content lookup needed to resolve
  `lints-config-blocked-by` markers on the explicit target.
...
- **A0-O only** (when `issue-scope` is `orphan-first`, or when
  ...
  `lints-config-roadmap-id` or
  `lints-config-blocked-by` markers.
...
- **A3 only**: a body-content search (e.g.,
  ...
  `lints-config-roadmap-id` marker when checking
  `lints-config-blocked-by` dependency markers (see A3
...
- All dependency issues are closed or otherwise completed. Two forms:
  ...
  `<!-- lints-config-blocked-by: {roadmap-id} -->` markers —
  find the issue whose body contains a matching
  `<!-- lints-config-roadmap-id: {roadmap-id} -->`; treat as
...
`<!-- lints-config-autopilot-suitability: N -->` footer, or the
...
Read the authored `<!-- lints-config-effort: S|M|L -->` footer
...
- **Roadmap identity** (`lints-config-roadmap-id`): in the
...
- **Sequential dependency** (`lints-config-blocked-by`): in an
  issue body — this issue **cannot start until** the roadmap with the
  matching `roadmap-id` is closed.

**Do not use `lints-config-blocked-by` to group sub-tasks under
