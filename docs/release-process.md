...
above goes in that package's `CHANGELOG.md`. Every other changed path
(root config, `.github/`, `docs/`, `scripts/`, `test/`, root
`README.md`, etc.) goes in the root `CHANGELOG.md`. A release that
touches multiple packages plus root gets one entry per affected file
group, synthesized as a concise, user-facing bullet from the
commits/PRs touching it — not one entry per raw commit.
...
`CHANGELOG.md` (dated at release time), leaves `## [Unreleased]`
present but empty for the next cycle, and includes those
`CHANGELOG.md` changes in the same release-prep commit/PR that bumps
the version.
