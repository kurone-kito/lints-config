...
3. Apply A3's readiness bullets to the target — no configured
   ...
   `lints-config-blocked-by` marker, both resolved the
...
- **Roadmap identity** (`lints-config-roadmap-id`): placed in
...
- **Sequential dependency** (`lints-config-blocked-by`): placed
