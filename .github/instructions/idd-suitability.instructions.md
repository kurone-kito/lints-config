...
The numeric `<!-- lints-config-autopilot-suitability: N -->`
...
`<!-- lints-config-autopilot-suitability: N -->` authoring
convention:

```markdown
<!-- lints-config-triage-verdict: <outcome> -->
```
...
`lints-config-triage-verdict` marker. Apply the same
